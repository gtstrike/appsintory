...
public class GajiController {
    ...
    public void tampilkanFormLihatKaryawan() {
        ...
        formLihatKaryawan =
                new FormLihatKaryawan(null, true);

        if (karyawan.bacaData()) {

            formLihatKaryawan.tampilkanData(
                    karyawan.getList()
            );

            formLihatKaryawan.setVisible(true);

            if (!formLihatKaryawan
                    .getKtpDipilih()
                    .equals("")) {

                if (karyawan.baca(
                        formLihatKaryawan.getKtpDipilih())) {
    ...
    public void tampilkanFormLihatPekerjaan() {
        ...
        formLihatPekerjaan =
                new FormLihatPekerjaan(null, true);

        if (pekerjaan.bacaData()) {

            formLihatPekerjaan.tampilkanData(
                    pekerjaan.getList()
            );

            formLihatPekerjaan.setVisible(true);

            if (!formLihatPekerjaan
                    .getKodePekerjaanDipilih()
                    .equals("")) {
    ...
    public void simpan(
        ...
        if (!ktp.getText().equals("")) {
            ...
            Object[][] listGaji =
                    new Object[gajiTable.getRowCount()][4];

            for (int i = 0;
                    i < gajiTable.getRowCount();
                    i++) {

                listGaji[i][0] =
                        gajiTable.getValueAt(i, 0);

                listGaji[i][1] =
                        gajiTable.getValueAt(i, 2);

                listGaji[i][2] =
                        gajiTable.getValueAt(i, 3);

                listGaji[i][3] =
                        gajiTable.getValueAt(i, 4);
            }
